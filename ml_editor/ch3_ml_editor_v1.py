# -*- coding: utf-8 -*-
"""
Created on Mon Dec 14 11:08:41 2020
...
def clean_input(text):
    ...
    :return: Sanitized text, without ascii characters
    '''
...
def preprocess_input(text):
    ...
    :return: Text  ready to be fed into analysis, by having sentences and
    words tokenized
    '''
...
def get_reading_level_from_flesh(flesch_score):
    '''
...
def count_sentence_syllables(tokens):
    ...
    # Our tokenizer leaves punctuation as a separate word, so we filter for it 
    punctuation = '.,!?/'
    return sum([
            count_word_syllables(word)
            for word in tokens if word not in punctuation
    ])
...
def get_suggestions(sentence_list):
    ...
    flesch = '%d syllables, %.2f flesch score: %s' % (
            number_of_syllables,
            flesch_score,
            get_reading_level_from_flesh(flesch_score)
    )    
